...
export default {
  ...
  methods: {
    ...
    sendDelete () {
      let self = this
      this.$api.deleteItem(self.deleteAim)
      .then((response) =>{         
    ...
    sendNewQuest () {
      ...
      if (iserror) {
        ...
        this.$api.addItem(self.newQuest)
        .then((response) =>{         
    ...
    init_socket() {
      const socket = this.$io.connect('//loc.firelocater.top')
      socket.on('connect', () => {
        const token = window.localStorage.getItem('token')
        socket.emit('authenticate', {token})
      })
      .on('authenticated', () => {
        socket.on('map', this.wbMap)
      })
      .on('unauthorized', (msg) => {
